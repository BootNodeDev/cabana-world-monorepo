...
export const useLastDrawWinnersByTier = () => {
  ...
  const { data: lastDraw, isFetched } = useQuery({
    ...
    queryFn: async () => {
      ...
      const lastDrawResult = await fetch(subgraphUrl, { method: 'POST', headers, body: lastDrawQuery })
      const lastDrawData = await lastDrawResult.json()
      ...
      const prizeClaimsResult = await fetch(subgraphUrl, { method: 'POST', headers, body: prizeClaimsQuery })
      const prizeClaimsData = await prizeClaimsResult.json()
      const prizeClaims = prizeClaimsData?.data?.prizeClaims || []
      ...
      return {
        drawId: lastDrawId,
        prizeClaims: prizeClaims.map((claim: any) => ({
          id: claim.id,
  ...
  const winnersByTier = useMemo(() => {
    ...
    lastDraw.prizeClaims.forEach((claim) => {
      if (claim.payout > 0n) {
